...
BEGIN {
  ...
  print mt "\n" mt "Behälteranschlüße:" >> TMP_DIR "/" UUID ".result.txt"
...
#  if (ps == 0 && (arr1[12] == "3001" || arr1[12] == "3002")) {
  if (ps == 0 && ke300x ~ /^[3][0][0]/) {
     found = 1
     print mt "Filtergehäuse ist nicht im Lieferumfang" >> TMP_DIR "/" UUID ".result.txt"
  }

  if (ps == 0 && ke300x !~ "300") {
     found = 1
     print mt "Sondersuaführung: siehe Endnummer" >> TMP_DIR "/" UUID ".result.txt"
  }
}
...
END {
  ...
  if (arr1[1] ~ "SH1")
     dt = 200
  print mt "\n" mt "Auslegungsdaten: "                  >> TMP_DIR "/" UUID ".result.txt"
  print mt "Auslegungstemperatur: " dt " Grad C"        >> TMP_DIR "/" UUID ".result.txt"
  print mt "Auslegungsdruck: " find_pressure(pn) " bar" >> TMP_DIR "/" UUID ".result.txt"
...
func print_values(fsr,port_size,pn,material,err_msg) {
  ...
  print mt "### port_size     : [" port_size "]"           >> TMP_DIR "/" UUID ".result.txt"
  print mt "### PN            : [" pn " bar ]"             >> TMP_DIR "/" UUID ".result.txt"
  print mt "### material      : [" material "]"            >> TMP_DIR "/" UUID ".result.txt"
  print mt "#############################################" >> TMP_DIR "/" UUID ".result.txt"

  print mt "Err.code 0401 - " err_msg                      >> TMP_DIR "/" UUID ".errlog.txt"
}
...
arr_pn[9]="400"
 ...
 if (pn in arr_pn) {
   ...
   print mt "######## PN = [" pn "] bar NOT FOUND in DB. PLS CHECK INPUT"        >> TMP_DIR "/" UUID ".result.txt"
   print mt "Err.code 0302 - PN = [" pn "] bar NOT FOUND in DB. PLS CHECK INPUT" >> TMP_DIR "/" UUID ".errlog.txt"
 }
}
...
arr_pn[9]="DN200"
 ...
 if (dn in arr_pn)
   ...
   print mt "######## DN = " dn " bar NOT FOUND in DB. PLS CHECK INPUT"        >> TMP_DIR "/" UUID ".result.txt"
   print mt "Err.code 0302 - PN = " pn " bar NOT FOUND in DB. PLS CHECK INPUT" >> TMP_DIR "/" UUID ".errlog.txt"
 }
}
