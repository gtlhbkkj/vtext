...
BEGIN {
  ...
  print mt "\n" mt "Filterantrieb:"  >> TMP_DIR "/" UUID ".result.txt"
}
...
if (found == 0 && $2 ~ motor_code) {
    split($2, arr2, "_!_")
    print mt arr2[2] >> TMP_DIR "/" UUID ".result.txt"
    for (i = 2; i <= NF; i++) {
...
END {
  ...
  # Kompletteinsatz -3001
  if (ke300x == "3001") {
    print mt "OHNE Antrieb im Lieferumfang" >> TMP_DIR "/" UUID ".result.txt"
    print mt "OHNE Motorbock"               >> TMP_DIR "/" UUID ".result.txt"
  }
  ...
  if (found == 0) {  # В конце, если found все еще 0
    ...
    print mt "####################################################" >> TMP_DIR "/" UUID ".result.txt"
    print mt "Err.code 0301 - " err_msg                             >> TMP_DIR "/" UUID ".errlog.txt"
  } else {
    string =  arr2[2] 
  }
  print string
