...
BEGIN {
  ...
  nr7                  = arr1[8]
#  if (nr7 != 0)
#    print "\n" mt "mit: " >> TMP_DIR "/" UUID ".result.txt"
}
...
if (found == 0 && nr7 != 0) {
  if ($2 ~ filter_series_search) {
     ...
     if (arr_f[2] ~ nr7) {
        found = 1
        print mt "\n" mt "mit:" $3 >> TMP_DIR "/" UUID ".result.txt"
     }
  }
} 
}
...
func print_values(fsr,nr7) {
  ...
  print mt "### third group   : [-." nr7 "...-]""]"        >> TMP_DIR "/" UUID ".result.txt"
  print mt "#############################################" >> TMP_DIR "/" UUID ".result.txt"

  print mt "Err.code 0702 - " err_msg                      >> TMP_DIR "/" UUID ".errlog.txt"
}
