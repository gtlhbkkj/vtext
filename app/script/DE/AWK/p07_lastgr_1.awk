...
BEGIN {
  ...
  print mt "\n" mt "Ausgestattet mit folgendem Zubehör, fertig montiert:" >> TMP_DIR "/" UUID ".result.txt"

  if (dpswitch == 0) {
    print mt "\n" mt "OHNE dP Schalter / dP Manometer"                     >> TMP_DIR "/" UUID ".result.txt"
    found = 1
  }
  ...
  if ("4,5" ~ dpswitch) {                  # PIS 3190 / 3175
    if (motor_no == 4 || agroup != "") {
         ...
         print mt "Warning - dP Manometer [-" dpswitch "....] is suitable for EX-Zone 2 only"   >> TMP_DIR "/" UUID ".errlog.txt"
    }
  }
...
if (found == 0 && $2 ~ filter_series_search) {
   ...
   if (arr_f[2] ~ material && arr_f[3] ~ dpswitch) {
      found = 1
      print mt arr_f[4] a_text >> TMP_DIR "/" UUID ".result.txt"
      for(i=3; i<=NF; i++) {
         if ($i ~ "_XXX_")
...
#  if (ke300x ~ /^[3][0][0]/)     # Kompletteinsat 300x
#     print mt "OHNE dP Schalter / Manometer"   >> TMP_DIR "/" UUID ".result.txt"
...
func print_values(fsr,material,dpswitch) {
  ...
  print mt "### dpswitch      : [" dpswitch "]"            >> TMP_DIR "/" UUID ".result.txt"
  print mt "#############################################" >> TMP_DIR "/" UUID ".result.txt"

  print mt "Err.code 0701 - " err_msg                      >> TMP_DIR "/" UUID ".errlog.txt"
}
