...
errlog_txt=${TMP_DIR}"/"${UUID}".errlog_ako.txt"

output=$(gawk -v txtdir=${txtdir} -v p01=${p01} -v p02=${p02} -v p03=${p03} -v p04=${p04} -v p05=${p05} -v p06=${p06} -v p07=${p07} -v p08=${p08} -v p09=${p09} -v p06material=${p06material} -v p06feinheit=${p06feinheit} -v r01=${r01} -v r04=${r04} -v r05=${r05} -v result_txt=${result_txt}  -v errlog_txt=${errlog_txt} -f ${awkdir}"print_vtext_ako.awk" ${txtdir}"data_01.txt" ${txtdir}"data_02.txt" > /tmp/awk.err.txt)
