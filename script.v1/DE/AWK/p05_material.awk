...
BEGIN {
  ...
  if (ke300x ~ /^[3][0][0]/)     # Kompletteinsat 300x
     print mt "\n" mt "Material Deckel und Innenteile:" >> "result.txt"
  else
     print mt "\n" mt "Material Behälter und Innenteile:" >> "result.txt"
}
...
END {
  ...
  if (filter_series_search == "AF113_G3" && "1,3" ~ filter_material) {
     print mt "Dichtungswerkstoffe: FKM (Viton), HDPE. Gleitlager: PTFE-Basis" >> "result.txt"
     } else {  
     print mt "Dichtungswerkstoffe: FKM (Viton). Gleitlager: PTFE-Basis"       >> "result.txt"
  }
}
...
func print_values(fsr,material,err_msg) {
  ...
  print mt "Err.code 0501 - " err_msg                      >> "errlog.txt"
}
