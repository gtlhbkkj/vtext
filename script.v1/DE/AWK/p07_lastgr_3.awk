  ...
  if ($5 ~ filter_series_search) {
     ...
     if (arr_f[2] ~ port_size && arr_f[3] ~ pn && arr_f[4] ~ material && arr_f[5] == ablass  ) {
        ...
        _portsize = substr(arr_p[1],3,length(arr_p[1]))
         if (ablass == 1)
            print mt "\n" mt "Hand-Kugelhahn " _portsize " für Ablass- bzw. Entleerungsanschluss" >> "result.txt"
         else 
            print mt "\n" mt "Automatik-Kugelhahn " _portsize " für Ablass- bzw. Entleerungsanschluss" text_ex  >> "result.txt"
...
END {
  if (found == 0 && ablass != 0) {  # В конце, если found все еще 0
   ...
   if (ke300x ~ /^[3][0][0]/ || ablass == 0)     # Kompletteinsat 300x
      print mt "OHNE Ablassventil"   >> "result.txt"
...
func print_values(fsr,port_size,pn,material,ablass) {
  ...
  print mt "### port size       : [-" port_size "..-"      >> "result.txt"
  print mt "### design pressure : [-.." pn ".-]"           >> "result.txt"
  print mt "### material        : [-..." material "-]"     >> "result.txt"
  print mt "### drain valve     : [-.." ablass "..]"       >> "result.txt"
  print mt "#############################################" >> "result.txt"

  print mt "Err.code 0703 - " err_msg                      >> "errlog.txt"
}
