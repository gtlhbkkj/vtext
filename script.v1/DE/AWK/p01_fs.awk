...
BEGIN {
  ...
  if (arr1[12] == "3001")
    print mt "Komplettfiltereinsatz ohne Filtergehäuse, ohne Antrieb, auf dem Deckel montiert zur" >> "result.txt"
  if (arr1[12] == "3002")
    print mt "Komplettfiltereinsatz ohne Filtergehäuse, mit Antrieb, auf dem Deckel montiert zur" >> "result.txt"
...

{
  if (found == 0 && $1 ~ filter_series_search) {
    found = 1
    print mt "Baureihe - " filter_series_report " FILTRATON GROUP " $2 >> "result.txt"
    if (endnummer != "") 
       print mt endnummer >> "result.txt"
    if (atext != "") 
       print mt atext >> "result.txt"
  }
}
...
END {
  if (found == 0) {  # В конце, если found все еще 0
    ...
    print mt "####################################################" >> "result.txt"
    print mt "Err.code 0101 - " err_msg                             >> "errlog.txt"
  }
  RS = "\n"
  FS = "[[:space:]]+"
}
