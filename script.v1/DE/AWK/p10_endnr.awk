...
# начинается проход по файлу данных
{
  if (found == 0 && $0 ~ endnr && $0 ~ filter_series_search) {
     found = 1
     print "\n" mt                                 >> "result.txt"
     print mt "[-" endnr "] Sonderausführung:" >> "result.txt"
     for (i=4; i<=NF; i++)
...
END {
  if (found == 0) {  
    print mt                                                              >> "result.txt"
    print mt "### End Number [-" endnr "] NOT FOUND IN DB"                >> "result.txt"
    print mt "Err.code 1001 - End Number [-" endnr "] NOT FOUND IN DB"    >> "errlog.txt"
  }
  RS = "\n"
  FS = "[[:space:]]+"
