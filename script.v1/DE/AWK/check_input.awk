...
print mt "-----------------------------------------------------------------------------" >> "errlog.txt"
print mt "ERROR LOG FOR FILTER : " filter_name >> "errlog.txt"
print mt "-----------------------------------------------------------------------------" >> "errlog.txt"

if (no_of_parts > 4) {
#  print mt "Err. 0011 - TOO MANY BLANK SPACES [ ] IN THE FILTER NAME" >> "errlog.txt"
  print mt "Err. 0011 - TOO MANY BLANK SPACES [ ] IN THE FILTER NAME" >> "errlog.txt"
  error_code = 1
}
...
if ( !(filter_name ~ /^[^\/]*\/[^\/]*$/)) {    # скопировал из гугла
  gsub(/\//,"[/]",filter_name)
  print mt "Err. 0021 - YOU MUST HAVE 1 SINGLE PRESENCE OF SLASH CHARACTER [/] IN THE FILTER NAME"  >> "errlog.txt"
  error_code = 1
}
...
if (! ((parts_slash[1] ~ /^[A][F][0-9]{4,5}[\-][0-9]{3,4}[\-][0-9]{5}[A-Z]{0,2}$/) || (parts_slash[1] ~ /^[A][F][0-9]{4,5}[\-][0-9]{3,4}[\-][0-9]{5}[\-][0-9]{4}[A-Z]{0,2}$/) || (parts_slash[1] ~ /^[A][F][0-9]{4,5}[\-][0-9]{3,4}[\-][0-9]{5}[\-][0-9]{4}[\-][0-9]{4}[A-Z]{0,2}$/) ))  {
  print mt "Err. 0031 - WRONG INPUT FORMAT FOR FILTER NAME: [" parts_slash[1] "]/" parts_slash[2] >> "errlog.txt"
  error_code = 1
}
...
if (no_of_groups > 2) {
    ...
    for (i=2; i<no_of_groups; i++) {
        if (! ( arr[i] ~ /^[A][1][3-5]$/ || arr[i] == "A23" || arr[i] ~ /^[K][I]{1,3}$/ || arr[i] ~ /^[K][I][V]$/ || arr[i] == "K0")) {
          print mt "Err. 0032 – WRONG INPUT FORMAT IN THE END OF FILTER NAME: [" arr[i] "]" >> "errlog.txt"
          error_code = 1
        }
    }
   ...
   if (no_of_groups == 4 && arr[2] == arr[3]) {
      print mt "Err. 0033 – AVOID DUPLICATION - TWO IDENTICAL GROUPS IN THE END OF FILTER NAME: [" arr[2] "]" >> "errlog.txt"
      error_code = 1
   }
}
...
if (element !~ /^[A][F][0-9]{4,6}[\-][0-9]{3}$/ && element !~ /^[A][F][0-9]{4}[\-][0-9]{3}[S][P]$/) {
  print mt "Err. 0034 - WRONG INPUT FORMAT OF THE FILTER ELEMENT [" element "]"  >> "errlog.txt"
  error_code = 1
}
...
if (fg_series !~ filter_series) {
  print mt "Err. 0041 - FILTER SERIES [" filter_series1 "] IS NOT FOUND in THE DB"  >> "errlog.txt"
  error_code = 1
}
...
if ((rsf ~ filter_series) && (length(arr_groups[1]) !=7)) {
  print mt "Err. 0051 - RSF FILTER SERIES CODE [" arr_groups[1] "-] IS WRONG"  >> "errlog.txt"
  error_code = 1
}

if ((rsf !~ filter_series) && (length(arr_groups[1]) !=6)) {
  print mt "Err. 0052 - KSP FILTER SERIES [" arr_groups[1] "-] IS WRONG"  >> "errlog.txt"
  error_code = 1
}
...
if (einsatzgroup != "" && endnrgroup != "") {
   ...
   if (einsatzgroup !~ /^[3][0][0]/ && endnrgroup !~ /^[3][0][0]/) {
      print mt "Err. 0061 – ONE OF THESE GROUPS MUST BE END NUMBER -300x: [-" einsatzgroup "], [-" endnrgroup "]" >> "errlog.txt"
      error_code = 1
   }

   if (einsatzgroup ~ /^[3][0][0]/ && endnrgroup ~ /^[3][0][0]/) {
      print mt "Err. 0062 – BOTH OF THESE GROUPS CAN NOT BE -300x: [-" einsatzgroup "], [-" endnrgroup "]" >> "errlog.txt"
      error_code = 1
   }

   if (einsatzgroup == endnrgroup) {
      print mt "Err. 0063 – BOTH OF THESE GROUPS CAN NOT BE SAME: [-" einsatzgroup "], [-" endnrgroup "]" >> "errlog.txt"
      error_code = 1
   }
...
if ( einsatzgroup ~ /^[3][0][0][1-2]$/ || endnrgroup ~ /^[3][0][0][1-2]$/) {
   if (substr(arr_groups[2],1,1) != 0) {
     ...
     print mt "Err. 0064 - KPL.EINSATZ! PORT SIZE IN THE SECOND GROUP MUST BE -[0]" part2_gr2 "- INSTEAD of -[" part1_gr2 "]" part2_gr2 "- "  >> "errlog.txt"
     error_code = 1
  }

  if (arr_groups[3] != "00000") {
    # возможно кроме некоторых RSF фильтров
    print mt "Err. 0065 - KPL.EINSATZ! THIRD GROUP MUST CONTAIN ZERO NUMBERS ONLY -[00000]- INSTEAD OF [-" arr_groups[3] "-]"  >> "errlog.txt"
    error_code = 1
  }
...
if (found == 0) {
    print mt "Err. 0071 - FILTER ELEMENT SERIES [" element_code "-] DOES NOT FIT FILTER SERIES [" filter_series1 "]"  >> "errlog.txt"
    error_code = 1
}
...
if (rsf ~ filter_series){
  if ("001,002,003,004,006,008,010,013" !~ element_end){
    print mt "Err. 0081 - CHECK FILTER ELEMENT FINENESS " element_code "-[" element_end "]"  >> "errlog.txt"
    error_code = 1
  }
} else {
  if ("003,004,005,006,008,010,013,016,020,025,030,036,050,070,100,200,300,400,500" !~ element_end){
    print mt "Err. 0081 - CHECK FINENESS OF FILTER ELEMENT: " element_code "-[" element_end "]"  >> "errlog.txt"
    error_code = 1
  }
}
...
if (motor_no == "0") {
   # no ENDNUMMER
   if (einsatzgroup == "" && endnrgroup == "") {
      print mt "Err. 0091 - IF THE MOTOR = 0, YOU MUST HAVE A GROUP WITH ENDNUMBER " >> "errlog.txt"
      error_code = 1
   }

   if (einsatzgroup != "3001" && endnrgroup == "") {
      print mt "Err. 0092 - MOTOR = 0, NOT KOMPLETTEINSATZ -3001, NO ENDNUMBER " >> "errlog.txt"
      error_code = 1
   }
} else {
   ...
   if (motor_no == "3" && (agroup ~ /^[A][1][3-5]$/ || arr[i] == "A23"))  {
      print mt "Err. 0093 - MOTOR = [3], BUT ATEX [" agroup "] IN THE END " >> "errlog.txt"
      error_code = 1
   }

   if (agroup == "A15" && motor_no != "7") {
      print mt "Err. 0094 - ONLY MOTOR = 7 IS SUITABLE FOR ATEX [" agroup "]" >> "errlog.txt"
      error_code = 1
   }

   if (einsatzgroup == "3001" || endnrgroup == "3001") {
      print mt "Err. 0095 - MOTOR != 0, BUT COMPLETE FILTER INSERT -3001 (WITHOUT MOTOR) " >> "errlog.txt"
      error_code = 1
   }
...
if (dp != 0) {
   if (motor_no == 4 || agroup != "") {
      if ("1,2,3,6,7,8" ~ dp) {
         print mt "Err. 0095 - DP SWITCH [-" dp "....] IS NOT SUITABLE FOR ATEX FILTER" >> "errlog.txt"
         error_code = 1
      }
   }
}
