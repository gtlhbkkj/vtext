...
if (found == 0 && $2 ~ find_filter_element) {
    ...
    if (callnumber==1) {
      print mt "mit  " arr2[2]                                >> "result.txt"
      print mt "Typ: " filter_element "; " arr2[3] " " str_element >> "result.txt"
    }

    # второй вызов для серединки более полный текст
    if (callnumber==2) { 
       print mt "\n" mt "Filterelement:"       >> "result.txt" 
       print mt "Typ: " filter_element      >> "result.txt" 
       print mt arr2[3] ": " str_element textsp >> "result.txt" 
       for (i=3; i<=NF; i++)
...
END {
  if (found == 0) {  # В конце, если found все еще 0
    ...
    print mt "####################################################" >> "result.txt"
    print mt "Err.code 0201 - " err_msg                             >> "errlog.txt"
  }
...
func find_felement(filter_series_search, filter_element, callnumber) {
  ...
  if ("AF112_G2, AF132_G2, AF113_G3, AF122_G1, AF133_G3, AF172_G2, AF173_G3" ~ filter_series_search) {
    ...
    if (fineness in arr2) 
      ...
      if (callnumber == 1)
        print mt "Err.code 0202 - " fineness " µm - WRONG FINENESS SPECIFICATION" >> "errlog.txt"
    }
  }
...
func print_no_elements(filter_series_search) {
  ...
  for (i=1; i<=6; i++) {
    if (arr1[i] ~ substr(filter_series_search,1,5)) {
     ...
     split(arr2[2],arr3,",")
     print mt "Anzahl der Etagen  : " arr3[1] >> "result.txt"
     print mt "Elemente pro Etage : " arr3[2] >> "result.txt"
     print mt "Gesamtelementenzahl: " arr3[3] >> "result.txt"
    }
  }
}
